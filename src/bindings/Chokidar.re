...
[@mel.send] external onEventWithUnitCallback: (watcher, string, unit => unit) => unit = "on";
...
let onReady = (chokidar, callback) => chokidar->onEventWithUnitCallback("ready", callback);

let onAdd = (chokidar, callback) => chokidar->onEventWithUnitCallback("add", callback);
