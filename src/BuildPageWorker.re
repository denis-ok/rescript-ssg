...
let workerOutput: workerOutput =
  ...
  ->Js.Array.map(
      ~f=
        (page: BuildPageWorkerT.workerPage) => {
          ...
          let () = page.globalValues->Belt.Option.forEach(globalValues => GlobalValues.unsafeAddJson(globalValues));

          logger.debug(() => Js.log2("[Worker] Trying to import page module: ", page.modulePath));
          ...
          let pageWrapperModule =
            ...
            | Some({modulePath, _}) =>
              logger.debug(() => Js.log2("[Worker] Trying to import page wrapper module: ", modulePath));
              ...
              let newPage: PageBuilder.page = {
                  ...
                  | (Some({component, modulePath}), Some(wrapperModule)) =>
                    ...
                    | WrapperWithChildren =>
                      Some({
                        component:
                          WrapperWithChildren(
                            children => React.createElement(wrapperModule##make, {"children": children}),
                          ),
                        modulePath,
                      })
                ...
                component: {
                  ...
                  | BuildPageWorkerT.ComponentWithoutData =>
                    ComponentWithoutData(React.createElement(module_##make, Js.Obj.empty()))
          ...
          ->Promise.catch(error => {
              logger.info(() => {
                Js.Console.error2({j|[Worker] $(pageInfo) Unexpected promise rejection, please check:|j}, error)
              });
              let result = Belt.Result.Error(page.path);
  ...
  ->Promise.all
  ->Promise.map(pages => {
      Js.log2("[Worker] Job done! Duration:", Performance.durationSinceStartTime(~startTime));
